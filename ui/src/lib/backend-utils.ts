...
// MCP backends defined inline in rawConfig get an auto-generated hierarchical
// name like "agentgateway/agentgateway-proxy/bind/80/http/default/my-route/backend0".
// Typed AgentgatewayBackend CRs use a clean "namespace/name" form without
// these path segments, so "/bind/" reliably identifies the auto-generated case.
const isAutoGeneratedBackendName = (name: string | undefined): boolean =>
  !!name && name.includes("/bind/");

// Get backend name for display
export const getBackendName = (backend: Backend): string => {
  // name already includes namespace prefix (e.g., "namespace/name")
  if (backend.mcp) {
    if (backend.mcp.name && !isAutoGeneratedBackendName(backend.mcp.name)) {
      return backend.mcp.name;
    }
    // Fall back to target names for rawConfig-defined backends.
    ...
    if (targetNames.length > 1) return `MCP: ${targetNames.join(", ")}`;
    return backend.mcp.name || "MCP Backend";
  }
