...
// Helper which extracts a clean semver from `git describe`. Mirrors the
// kindred-mcp-gateway Jenkinsfile so chart + image versions stay in lockstep
...
genericPod([
    ...
    stage('Build & Push Docker Image') {
        container('docker') {
            ...
            docker.withRegistry("https://${jfrogRegistry}", 'artifactory-docker-deploy') {
                // Buildx is not used here; the multi-stage Dockerfile targets
                // linux/<TARGETARCH>. Jenkins agents are amd64, matching the
                // GHCR build-image.yml workflow. Cross-arch builds belong in a
                // separate buildx-enabled stage if/when needed.
    ...
    stage('Helm Package And Publish') {
        container('docker') {
            ...
            // mirroring the kindred-mcp-gateway flow. Without this the chart
            // would render with empty image fields.
            sh """
    ...
    stage('Tag Release') {
        if (env.BRANCH_NAME == 'master') {
            withCredentials([usernamePassword(
                ...
                // Origin URL is repo-specific — the agentgateway fork lives on
                // GitHub upstream but the corp mirror at Bitbucket is what
                // Jenkins pushes to. Adjust the URL if/when the corp mirror
                // location changes.
                sh """
